...
<br>따라서 위 docling의 경우를 기준으로 설명하면 다음과 같다. (파일을 읽는 부분 ~ 각 부분을 어떻게 판단하고 보정하는지 까지 살펴보면)

#### parse PDF pages 단계에서는 

1. pdf 파일을 적절하게 읽어 다음과 같은 트리 형태로 잘 파싱해내는 부분과 ([[pdf 파일 구조#pdf 파일 읽기]]에서 언급했듯이)
2. pypdfium2를 이용해서 각 요소가 차지하는 영역을 이미지로 만들어 내는 부분이 있다.
...
1. 이미지 정보를 이용해 적절하게 각 부분이 어떤 것인지 판단해낸다
...
<mark style="background:#d4b106">=> 테이블 뿐만 아니라 Layout Analysis 에서의 결과를 바탕으로 보완하는 단계를 둔다.</mark>
### 개선해볼 여지...?
